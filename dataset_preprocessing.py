...
torch.save(train_dataset_loader, './data/img_cropped/train_transformed_dataset.pt')

# for i, data in enumerate(train_dataset_loader):
#   torch.save(data[0], './data/img_cropped/train_transformed_img_{}'.format(i))
N_mini_batches = len(train_dataset_loader)
...
# for i, data in enumerate(val_dataset_loader):
torch.save(val_dataset_loader, './data/img_cropped/val_transformed_dataset.pt')


test_dataset_loader   = torch.utils.data.DataLoader(
    CelebAttributes(partition='test', data_dir='./data',
                    image_transform=preprocess_data),
                    batch_size=100, shuffle=True)
print(len(test_dataset_loader))
# for i, data in enumerate(test_dataset_loader):
torch.save(test_dataset_loader, './data/img_cropped/test_transformed_dataset.pt')
